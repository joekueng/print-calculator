...
describe('PrintItemControlsComponent', () => {
  it('renders translated controls, commits quantity on blur, and selects material variants', async () => {
    await TestBed.configureTestingModule({
      ...
      providers: [{ provide: LanguageService, useValue: { selectedLang: () => 'it' } }],
    }).compileComponents();
    ...
    fixture.componentRef.setInput('variants', [{
      id: 12, name: 'Nero', colorName: 'Nero', colorLabelIt: 'Nero', hexColor: '#000000',
      finishType: 'GLOSSY', stockSpools: 1, stockFilamentGrams: 1000, isOutOfStock: false,
    }]);
    ...
    expect(translate.instant('CHECKOUT.ERR_UPDATE_ITEM')).not.toBe('CHECKOUT.ERR_UPDATE_ITEM');
    ...
    element.querySelector<HTMLButtonElement>('.trigger')!.click(); fixture.detectChanges();
    element.querySelector<HTMLElement>('.color-item')!.click();
    expect(color).toHaveBeenCalledWith({ colorName: 'Nero', filamentVariantId: 12 });
    fixture.componentRef.setInput('disabled', true); fixture.detectChanges();
    await fixture.whenStable();
    expect(input.disabled).toBeTrue();
    expect(element.querySelector<HTMLButtonElement>('.trigger')!.disabled).toBeTrue();
  });
});
